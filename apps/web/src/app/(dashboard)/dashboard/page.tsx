...
import { CORPUS, QUERIES, STATS } from '@/lib/mock';
...
export default function DashboardPage() {
  ...
  const flagged = CORPUS.filter(c => c.freshnessStatus === 'amended' || c.freshnessStatus === 'repealed').slice(0, 4);
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {QUERIES.slice(0, 5).map((q, i) => (
                <div key={q.id} onClick={() => router.push(q.kind === 'chat' ? '/chat' : '/search')}
                  ...
                  <span style={{ flex: 1, fontSize: 13.5, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{q.text}</span>
                  ...
                  <span style={{ fontSize: 11.5, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', minWidth: 40, textAlign: 'right' }}>{q.time}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginTop: 12 }}>
                {([['green', t('dash.stat.ready'), S.ready], ['amber', t('import.step.process'), S.pending], ['red', 'Error', S.error]] as [string, string, number][]).map(([c, lb, v]) => (
                  <div key={lb} style={{ display: 'flex', alignItems: 'center', gap: 9, fontSize: 13 }}>
                    <span className={'badge-dot dot-' + c} style={{ width: 9, height: 9 }}></span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {flagged.map(c => (
                <div key={c.id} onClick={() => router.push('/freshness')}
                  ...
                  <span style={{ flex: 1, minWidth: 0, fontSize: 12.5, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.fileName}</span>
                  <FreshBadge status={c.freshnessStatus} />
                </div>
              ))}
            </div>
          </div>
